...
import com.washing_blanket_prd.washing_blanket.databinding.CalendarHeaderBinding;
import com.washing_blanket_prd.washing_blanket.databinding.DayItemBinding;
import com.washing_blanket_prd.washing_blanket.databinding.EmptyDayBinding;
import com.washing_blanket_prd.washing_blanket.databinding.CalendarWeekBinding;
import com.washing_blanket_prd.washing_blanket.viewmodel.CalendarHeaderViewModel;
import com.washing_blanket_prd.washing_blanket.viewmodel.CalendarViewModel;
import com.washing_blanket_prd.washing_blanket.viewmodel.CalenderWeekModel;
import com.washing_blanket_prd.washing_blanket.viewmodel.EmptyViewModel;
...
public class CalendarAdapter extends ListAdapter<Object, RecyclerView.ViewHolder> {
    ...
    public int getItemViewType(int position) { //뷰타입 나누기
        ...
        if (item instanceof Long) {
            ...
            Log.i("널일때 item의 타입은", item +" "+ position+"  "+ item.toString().length());
            return EMPTY_TYPE; // 비어있는 일자 타입
        } else if (item instanceof String && !item.equals("empty")) {
            Log.i("주의 이름일때 item의 타입은", item +" "+ position+"  "+ item.toString().length());
            return WEEK_TYPE; // 비어있는 일자 타입
        } else {
            Log.i("요일일때 item의 타입은", item +" "+ position+"  "+ item.toString().length());
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        } else if (viewType == WEEK_TYPE) {
            CalendarWeekBinding binding =
                    DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()), R.layout.item_calender_week, parent, false);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (viewType == HEADER_TYPE) { //날짜 타입 꾸미기
            ...
            Object item = getItem(position);
            CalenderWeekModel model = new CalenderWeekModel();
            if (item instanceof String && !item.equals("empty")) {
                model.setWeekCalendar((String) item);
            }
            holder.setViewModel(model);
    ...
    private class WeekViewHolder extends RecyclerView.ViewHolder {// 요일 타입 ViewHolder
        private CalendarWeekBinding binding;

        private WeekViewHolder(@NonNull CalendarWeekBinding binding) {
            super(binding.getRoot());
            this.binding = binding;
        }

        private void setViewModel(CalenderWeekModel model) {
            binding.setModel(model);
            binding.executePendingBindings();
        }
    }
